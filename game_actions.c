...
Status game_actions_recruit(Game *game)
{
  ...
  if (!(space = game_get_space(game, space_id)))
  {
    return ERROR;
  }
  ...
  if (!(character = game_get_character(game, char_id)))
  {
    return ERROR;
  }
  ...
  /* El personaje a reclutar debe ser el que comparte sala con el jugador. */
  if (strcmp(name, arg) != 0 || character_get_friendly(character) == FALSE)
  {
    return ERROR;
  }
