...
Status graphic_engine_get_exits_str(Game *game, Space *space, char *str, size_t size);
...
Status graphic_engine_get_exits_str(Game *game, Space *space, char *str, size_t size)
{
    Directions dirs[] = {N, S, E, W, UP, DOWN};
    int i;
    BOOL first = TRUE;

    if (!game || !space || !str || size == 0)
    {
        return ERROR;
    }
    ...
    {
        if (game_get_connection(game, space_get_id(space), dirs[i]) == NO_ID)
        {
            continue;
        }
        ...
        strncat(str, link_direction_to_string(dirs[i]), size - strlen(str) - 1);
        first = FALSE;
    }
...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game, Status last_cmd_status, BOOL paint_cmd)
{
    ...
    if (act && graphic_engine_get_exits_str(game, act, exits, sizeof(exits)) == OK)
    {
        sprintf(str, " Exits: %s", exits);
        screen_area_puts(ge->descript, str);
    }
